const getData = async () => fetch('http://api.kvikmyndir.is/upcoming', {
  method: 'GET',
  headers: {
    'Content-type': 'application/json; charset=UTF-8',
    'x-access-token': 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..9zr5lkIbfCRmv3GPTMTtKqE82YZAp4owOJf-K4YEb4Q',
  },
}).then((response) => response.json());

const getComingSoonSuccess = (currentSoon) => ({
  type: 'getComingSoon',
  payload: currentSoon,
});

export const getComingSoon = () => async (dispatch) => {
  try {
    const currentSoon = await getData();
    dispatch(getComingSoonSuccess(currentSoon));
  } catch (error) {
    console.log(error);
  }
};
